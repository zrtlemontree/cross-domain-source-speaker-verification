...
class Inferencer(BaseAgent):
    ...
    def load_wav_data(self, source_path, target_path, out_path):
        ...
        os.makedirs(os.path.join(out_path, 'conv'), exist_ok=True)
    ...
    #  inference
    # ====================================================
    def inference(self, source_path, target_path, target_root, out_path, seglen, cnt):
        ...
        with torch.no_grad():
            for i, source in enumerate(sources):
                ...
                for j, target in enumerate(targets):
                    ...
                    source_basename = os.path.basename(source.path).split('.')[0]
                    target_basename = os.path.basename(target.path).split('.')[0]
                    target_base_path = os.path.split(target_path)[0]

                    output_basename = f'{source_basename}_to_{target_basename}'
                    output_wav = os.path.join(target_base_path.replace(target_root, out_path), output_basename+'.wav')
                    output_plt = os.path.join(target_base_path.replace(target_root, out_path), output_basename+'.png')
                    ...
                    # conversion_name = source_basename.split("_")[0] + "_to_" + target_basename.split("_")[0]
                    # np.save(os.path.join(out_path, 'conv', f'{conversion_name}.npy'), dec.squeeze().cpu().numpy())
        logger.info(f'cnt: {cnt} The generated files are saved to {output_wav}.')